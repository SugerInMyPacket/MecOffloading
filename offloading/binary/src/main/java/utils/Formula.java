...
public class Formula {
     ...
     *
     * @param taskList         ： 任务列表
     * @param taskCostTimeList ： 任务花费时间list
     * @param taskUSSList      任务满意度 list
     * @return
     */
    public static void getSatisfaction4TaskList(List<Task> taskList,
                                                List<Double> taskCostTimeList,
                                                List<Double> taskUSSList) {
        // log.info("========================= 计算任务满意度List =========================");
        int len = taskCostTimeList.size();
        ...
        for (int taskIndex = 0; taskIndex < len; taskIndex++) {
            ...
            if (currTaskCostTime <= currTaskMaxDeadline) {
                double exp = (currTaskCostTime - currTaskDeadline) / currTaskMaxDeadline * currTaskPrior / taskMaxPrior;
                // 用户满意度计算阈值 θ
    ...
    public static void calculateEnergy4TaskList(List<Task> taskList,
                                                List<Integer> unloadArr,
                                                List<Integer> resAllocArr,
                                              /* List<Double> taskUplinkEnergyV2VList,
                                              List<Double> taskUplinkEnergyV2RList,
                                              List<Double> taskUplinkEnergyR2CList,
    ...
    public static void calculateEnergy4TaskList(List<Task> taskList,
                                                List<Integer> unloadArr,
                                                List<Integer> resAllocArr,
                                                List<Double> taskUplinkTimeV2VList,
                                                List<Double> taskUplinkTimeV2RList,
                                                List<Double> taskUplinkTimeR2CList,
    ...
    // TODO：计算当前时间槽开始时刻o的剩余计算资源
    public static int getRemainFreq(Object o) {
        return 0;
    }

    public static void updateRemainFreq(Object o) {
        // 更新 o 的剩余计算资源
    }
     ...
     * 计算每个任务的 uss
     *
     * @param taskMap          ========  key：编号 --> value：Task
     * @param unloadArr
     * @param resArr
     * @param taskCostTimeList : 每个任务花费的时间
     * @param ussList
     */
    public static void calculateTaskListUSS(Map<Integer, Task> taskMap,
                                            List<Integer> unloadArr,
                                            List<Integer> resArr,
                                            List<Double> taskCostTimeList,
                                            List<Double> ussList) {
        int len = taskMap.size();

        // 遍历，找到 p_max
        int priorMax = 0;
        for (int i = 0; i < len; i++) {
            Task currTask = taskMap.get(i);
            int currPrior = currTask.getP();
            priorMax = Math.max(currPrior, priorMax);
        }
     ...
     */
    public static void calculateTaskCostTime(List<Task> taskList, /* Map<Integer, Task> taskMap,*/
                                             List<Integer> unloadArr,
                                             List<Integer> resAllocArr,
                                             /* List<Double> taskUplinkTimeV2VList,
                                             List<Double> taskUplinkTimeV2RList,
    ...
    public static void calculateTaskExecTimeBySort(List<Task> taskList,
        ...
        for (int i = 0; i < len; i++) {
            taskExecTimeList.add(tasksExecTimeMap.get(i));
        }
    ...
    public static void calculateTaskTransTime4Uplink2RSU(List<Task> taskList,
        ...
        int vehicleNeed2RSUnums = getVehicleNums2RSU(unloadArr);
    ...
    public static double getCurrVehicleRateV2R(int vehicleID) {
        ...
        // double rateV2R = currVehicleChannelWidth * (1 - Math.abs(posX - 200) / 400);

        return rateV2R;

    }

    public static double getCurrVehicleChannelGain(int vehicleID) {
        double currVehicleChannelGain = 0.0;


        return currVehicleChannelGain;
    }

    public static int getVehicleNums2RSU(List<Integer> unloadArr) {
        int len = unloadArr.size();
        Set<Integer> hs = new HashSet<>();
        for (int i = 0; i < len; i++) {
            if (unloadArr.get(i) > 0) continue;
            hs.add(unloadArr.get(i));
        }
        return hs.size();
    }
    ...
    public static void calculateTaskTransTime4UplinkR2C(List<Task> taskList,
        ...
        for (int i = 0; i < len; i++) {
            if (unloadArr.get(i) != -1) {
                ...
                // todo: change
                currTaskUplinkTime = currTaskUplinkTime * Constants.TRANS_R2C_MULTI_HOP;  // 多跳
    ...
    public static void calculateTaskTransTime4V2V(List<Task> taskList,
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (vehicleOfCurrTask == objOffloadVehicle) {
                ...
                // 计算当前任务的传输时间
                // TODO: 比较车辆数目影响时的改动
                double currTaskTransTime = taskSize / rate4v2v / 20.0;
    ...
    public static double getTransRateR2V(int vehicleID) {
        ...
        double widthR2C = Constants.WIDTH_CHANNEL_RSU;
    ...
    public static double getTransRateV2O(int taskVehicleID, int objVehicleID) {
        ...
        double lengthV2O = getTransDistanceV2O(taskVehicleID, objVehicleID);
    ...
    public static double getTransDistanceV2O(int taskVehicleID, int objVehicleID) {
        double dis = 1.0;
        int posX_1 = (int) InitFrame.getVehicleList().get(taskVehicleID - 1).getPosX();
        int posX_2 = (int) InitFrame.getVehicleList().get(objVehicleID - 1).getPosX();
        if (Math.abs(posX_1 - posX_2) <= 10) dis = 1.0;
        else dis = Math.sqrt(Math.abs(Math.pow(posX_1, 2) - Math.pow(posX_2, 2)));
        // 计算规则：找到 ＞当前车辆信道增益的车辆
    ...
    public static void calculateTaskExecEnergy(List<Task> taskList,
        ...
        for (int i = 0; i < len; i++) {
            ...
            double freqAlloc4task = (double) freqAllocArr.get(i) / 10.0;
            ...
            // if (currTaskExecEnergy < 0.1 || currTaskExecEnergy > 20000) currTaskExecEnergy =0.1;
            // currTaskExecEnergy = (int) currTaskExecEnergy;
