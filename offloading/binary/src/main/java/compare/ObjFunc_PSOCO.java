...
public class ObjFunc_PSOCO {

    // 根据目标函数进行修改
    static double[] evaluate(int[] currUnloadArr, double[] freqAllocArr) {
        ...
        for (int i = 0; i < numDimensions; i++) {
            currParticleUnloadArr.add(currUnloadArr[i]);
        }
        ...
        // 修正卸载
        // RevisePolicy.reviseUnloadArr(taskList, vehicleList, rsu,
        //         currParticleUnloadArr, currParticleResAllocArr,
        //         currUssList, currEnergyList);
        RevisePolicy.reviseUnloadArr(taskList, vehicleList, rsu,
                currParticleUnloadArr, currParticleResAllocArr);
        // System.out.println("currParticleUnloadArr: " + currParticleUnloadArr);

        // 修正后的值返回
        for (int i = 0; i < numDimensions; i++) {
            currUnloadArr[i] = currParticleUnloadArr.get(i);
            freqAllocArr[i] = currParticleResAllocArr.get(i);
        }
