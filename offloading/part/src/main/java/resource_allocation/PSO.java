...
public class PSO {
    ...
    // KEYNOTE: numDimensions == taskSizeFromDB * 2
    ...
    // 个体pareto最优解集
    HashMap<Integer, List<double[]>> paretoPosIndiv;
    ...
    List<double[]> paretoUnloadRatioArr;
    ...
    public List<double[]> getParetoUnloadRatioArr() {
        return paretoUnloadRatioArr;
    }
    ...
    public PSO(int numParticles,
        ...
        this.paretoUnloadRatioArr = new ArrayList<>();
        ...
        this.currUnloadRatioArr = new ArrayList<>();
        for (int i = 0; i < numDimensions / 2; i++) {
            currUnloadRatioArr.add(currInputUnloadRatioArr.get(i));
        }

        this.currParticleUnloadArr = new ArrayList<>(currInputUnloadArr);
        this.currParticleUnloadRatioArr = new ArrayList<>(currInputUnloadRatioArr);

        // 初始化粒子属性
        for (int i = 0; i < numParticles; i++) {
            particles[i] = new Particle(numDimensions, bounds, currUnloadArr, currUnloadRatioArr);
            // 更新 pareto 解
    ...
    public PSO(int numParticles,
        ...
        for (int i = 0; i < numParticles; i++) {
            if (i < 0) {
                particles[i] = new Particle(numDimensions, bounds,
                        currUnloadArr, currUnloadRatioArr);
            } else {
                particles[i] = new Particle(numDimensions, bounds, currUnloadArr, currUnloadRatioArr);
            }
            // 更新 pareto 解
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            for (int i = 0; i < numParticles; i++) {
                ...
                for (int j = 0; j < numDimensions; j++) {
                    ...
                    newPosition[j]
                            = particles[i].position[j] + newVelocity[j];
                ...
                // 适应度值评估
                double[] newValue
                        = ObjFuncPSO.evaluate(particles[i].currParticleUnloadList,
                        particles[i].currParticleUnloadRatioList, newPosition);
                ...
                currParticleUnloadRatioArr = new ArrayList<>(particles[i].currParticleUnloadRatioList);
    ...
    public void addParetoUnloadRatioArr4CurrP(double[] unloadRatioArr) {
        paretoUnloadRatioArr.add(unloadRatioArr);
    }
    ...
    private void updateParetoFront(double[] position, double[] value) {
        ...
        double[] tempUnloadRatioArr = new double[currUnloadRatioArr.size()];
        for (int i = 0; i < currUnloadArr.size(); i++) {
            tempUnloadArr[i] = currParticleUnloadArr.get(i);
            tempUnloadRatioArr[i] = currParticleUnloadRatioArr.get(i);
        }
        // paretoUnloadArr.add(tempUnloadArr);
        addParetoUnloadArr4CurrP(tempUnloadArr);
        addParetoUnloadRatioArr4CurrP(tempUnloadRatioArr);
        ...
        for (int i = 0; i < toRemoveIndex.size(); i++) {
            ...
            paretoUnloadRatioArr.remove(del);
        }
    }
